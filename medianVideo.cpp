...
void processVideo(Mat &h_input, Mat &h_output){
    ...
    filter = cuda::createMedianFilter(CV_8UC1, 12);
    filter->apply(d_gray, d_resoult);
...
int main(int argc, char* argv[]){
    ...
    srcVideo.open("videos/median/originalMedian.mp4", codec, fps, Size(frame_width, frame_height)); 
    resoultVideo.open("videos/median/resoultMedian.mp4", codec, fps, Size(frame_width, frame_height), false); 
